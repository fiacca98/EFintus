...
import com.lynx.EFintus.marketing.classes.MetodoPagamento;

public class MetodoPagamentoDao extends GenericDao<MetodoPagamento> {

    private String TABLE_NAME = "metodo_pagamento";
	...
	Connection con = getConnection();
	PreparedStatement ps = con.prepareStatement("delete from " + getTableName() + " where metodoPagamento = ?");
	ps.setString(1, metodoPagamento.getMetodoPagamento());
	ps.executeUpdate();
	con.close();
    }
