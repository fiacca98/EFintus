...
public class ProduttoreDao extends GenericDao<Produttore> {
    ...
    @Override
    public Produttore get(long id) throws SQLException {
return null;
    }
    ...
    public List<Produttore> getAll() throws SQLException {
        ...
        while (rs.next()) {
            Produttore produttore = new Produttore(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
    ...
    public void delete(Produttore produttore) throws SQLException {
        Connection con = getConnection();
        PreparedStatement ps = con
                .prepareStatement("DELETE FROM" + getTableName()+"WHERE ID="+produttore.get_ID());
    ...
    @Override
    public Produttore fromResultSetToBean(Produttore produttore) {
        return null;
    }
